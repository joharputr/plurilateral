...
class Journal extends MY_Controller
{
    ...
    { 
		...
		$data['kegiatan'] = $this->db->query("SELECT * FROM kegiatan e LEFT JOIN dokumen d ON d.event = e.id $sql_where " )->result();
		$data['dokumen'] = $this->db->query("SELECT * FROM dokumen d WHERE d.tipe = 'jurnal'")->result();
